...
- **Localization:** Use `ngx-translate` v18 standalone provider `provideTranslateService` with `fallbackLang: 'en'`. Import static translation bundles (`localeEn` and `localeDe`) from `src/app/i18n/` to prevent async HTTP fetching flicker under SSR. Only translate the legal notice and privacy policy pages for legal compliance reasons. Keep all other UI elements, content, components, and pages strictly in English unless explicitly asked otherwise.
...
- **Deployment Scripts:** The `start` script in `package.json` MUST remain mapped to the compiled production SSR server (`node dist/dmh.dev-web/server/server.mjs`) to ensure compatibility with Railway deployment. Do NOT modify this to `ng serve`. For local development, run the dev server via `npx ng serve` or `npm run ng -- serve` instead.
